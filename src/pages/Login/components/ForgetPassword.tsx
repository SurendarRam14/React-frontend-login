...
const ForgetPassword: React.FC<LoginProps> = ({ setIsForgetPasswordComponent }) => {
    ...
    const handleForgetPassword = async (email: string, password: string) => {
        try {
            const response = await forgetPassword({ email, newPassword: password }).unwrap();
            console.log("Forget password response -", response);
            setIsForgetPasswordComponent(false);
        } catch (err) {
    ...
    const validateInputs = () => {
        ...
        if (isValid) {
            handleForgetPassword(email, password);
        }
    };
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0 }} >
                <FormControl>
                    <div style={{ display: 'flex', justifyContent: "flex-start", fontSize: "13px" }}>
                        ...
                        <span>
                            <a href="#" style={{ color: "blue", textDecoration: "underline", cursor: "pointer" }} onClick={() => { setIsForgetPasswordComponent(false) }}>
                                Back to login
                            </a>
                        </span>
                    </div>
                ...
                <FormControl>
                    <FormLabel className="custom-label" htmlFor="password">New Password</FormLabel>
                    <TextField
                        required
                        fullWidth
                        name="password"
